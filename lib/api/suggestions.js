...
export const suggestionAPI = {
  getProgress: (page = 1) => apiRequest(`/api/suggestions/progress?page=${page}`),
  updateProgress: (id, data) => apiRequest(`/api/suggestions/${id}/progress`, {
  ...
  voteSuggestion: async (suggestionId, value, identityVisibility = 'anonymous') => {
    return apiRequest(`/api/suggestions/${suggestionId}/vote`, {
      method: 'POST',
      body: JSON.stringify({ value, identityVisibility }),
    });
  },

  voteSolution: async (solutionId, value, identityVisibility = 'anonymous') => {
