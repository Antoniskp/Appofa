...
- **2026-05-21** — Enhanced Homepage with new sections: "Civic Questions", "My Community", and "Discovery" (people-focused). Refactored `PersonCard` and `OrganizationCard` into reusable components (`components/user/PersonCard.js`, `components/organizations/OrganizationCard.js`) and updated list pages to use them. Implemented dynamic alternating background colors for homepage sections.
- **2026-05-20** — Implemented `isPrimary` for Formations (migration `20260520000000-add-is-primary-to-formations.js`), refactored `FormationList.js` to remove name/category fallback logic, and updated `dreamTeamController.js` for field persistence. Enhanced admin message management in `app/admin/messages/[id]/page.js` with a response form and added automated platform+email notifications for message replies in `messageController.js`. Patched `brace-expansion` DoS vulnerability via `package.json` overrides.
- **2026-05-20** — Improved no-content country landing UX on `/country/[code]`: `components/locations/CountryFundingBanner.js` now renders a polished, action-oriented empty state with detected-country flag badge, explicit network/IP-region detection label, richer no-local-content guidance, support CTA (`/contribute`), optional donation CTA when country funding is active, and a dedicated diaspora CTA to Greece (`/country/GR`). `app/country/[code]/page.js` now always mounts the banner for no-content states and passes `countryCode` for flag rendering. Added focused coverage in `__tests__/country-funding-banner.test.js`.
- **2026-05-19** — Refined location detail IA to be participation-first: `app/locations/[slug]/page.js` now renders `#location-content` (tabs) directly under the header, keeps `polls` and `suggestions` tabs always visible, hides "Τοπικές πληροφορίες" entirely when empty, and no longer fetches role-count-only data for the removed top overview block. `components/locations/LocationHeader.js` now uses a single primary participation CTA (`Ψήφισε τώρα` or `Κάνε πρόταση`), demotes edit to a compact icon action near the title, and mutes zero-value stat tiles. `components/locations/LocationRelatedLocations.js` replaces the large hierarchy grid with compact related/nearby chips. `components/locations/LocationTabs.js` now renders polls in a compact 2-column grid and uses explicit `+ Ξεκίνησε ...` empty-state CTAs. Updated coverage in `__tests__/location-ui-localized-names.test.js`, `__tests__/location-phase3-ui.test.js`, and `__tests__/location-tabs-suggestion-anonymity.test.js`.
- **2026-05-19** — `proxy.js` now keeps `CF-IPCountry` as first choice but falls back to a backend `/api/geo/detect` lookup when no country cookie/header exists on non-skipped page requests, using new helper `lib/geo/proxyCountryDetection.js` with a short safe timeout and cookie persistence for successful fallback detections. Added focused coverage in `__tests__/country-redirect-middleware.test.js`.
- **2026-05-18** — Added suggestion-level creator anonymity (`Suggestion.hideCreator`) equivalent to polls: new migration `20260518162000-add-hide-creator-to-suggestions.js`; suggestion create/update APIs now accept/persist `hideCreator`; suggestion responses now sanitize hidden authors for guests/regular users while preserving visibility for owner/admin/moderator; organization suggestion and official-post suggestion serializers now honor `hideCreator`; suggestion create/edit forms now expose `Απόκρυψη δημιουργού`; suggestion card/detail/location-tab UI now show anonymous labels when creator is hidden. Added focused coverage in `__tests__/suggestions.test.js`, `__tests__/suggestion-card-layout.test.js`, `__tests__/suggestion-detail-avatar.test.js`, and `__tests__/location-tabs-suggestion-anonymity.test.js`.
- **2026-05-18** — Added embeddable public cards for polls, suggestions, and civic questions: new route `app/embed/[entityType]/[id]/page.js` renders responsive read-only cards via `components/embed/EntityEmbedView.js`; new `components/layout/AppShell.js` hides TopNav/Footer/CookieBanner on `/embed/*` so iframe views stay clean; `components/ui/ShareModal.js` now supports copying an embed URL and iframe code when `embedPath` is provided; poll/suggestion/civic-question detail pages now expose embed sharing. Added focused coverage in `__tests__/embed-utils.test.js`, `__tests__/share-modal-embed.test.js`, `__tests__/entity-embed-view.test.js`, and `__tests__/app-shell-embed.test.js`.
- **2026-05-17** — Replaced legacy boolean `User.searchable` with explicit `User.profileVisibility` (`hidden` / `registered` / `public`) using migration `20260517211000-add-profile-visibility-to-users.js` (`false→hidden`, `true→registered`, then remove runtime `searchable`). Registration step 3 now presents explicit visibility radio choices with “change later in profile” guidance; profile Settings privacy now edits `profileVisibility`; auth profile/search endpoints now use optional auth + visibility enforcement (guest: public only, authenticated: registered+public, owner/admin/mod can directly access hidden). Hidden profiles are excluded from discoverability/selection surfaces (search/unified-search/location user feeds/follow-lists/dream-team candidate checks), aligning position-voting/selection flows so hidden users cannot appear in search bars.
- **2026-05-17** — Added location page Phase 3 overview/navigation improvements without map functionality: new `components/locations/LocationOverviewPanel.js` surfaces summary cards for proposals, representatives, announcements, local media, community users, and sublocations with guided empty-state copy/links; new `components/locations/LocationRelatedLocations.js` adds explicit parent/sibling/child navigation blocks beyond the header chips; `app/locations/[slug]/page.js` now fetches sibling + role-count data, inserts anchored overview/local-info/roles sections, and shows a stronger local-info empty state; `components/LocationRoles.js` now supports an actionable public empty state; `components/locations/LocationTabs.js` now uses richer empty states for polls/news/articles/suggestions. Map support remains intentionally deferred to Phase 4. Added focused coverage in `__tests__/location-phase2-ui.test.js` and new `__tests__/location-phase3-ui.test.js`.
...
| CivicQuestion | `id`, `title`, `sourceType`, `category`, `deadline`, `locationId` | — |
...
- **Geo detection API**: use public `GET /api/geo/detect` (CF-IPCountry first, optional geoip-lite fallback) for lightweight country detection; `proxy.js` now uses it as the non-Cloudflare fallback on non-skipped page requests when no header/cookie country is available
