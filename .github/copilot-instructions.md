...
- **Users page (unified directory)**: `/users` is the single people directory; shows person profiles (public, all visitors) via `PersonsPanel` with infinite scroll + full filter bar (search/location/domain/expertise/claimStatus), and registered-user search (auth-required) via `RegisteredUsersPanel` with taxonomy cascade + pagination; tab toggle `🏛️ Πρόσωπα` / `👤 Εγγεγραμμένοι`; guests see persons tab + login/register CTA in users tab; `/discover-people` and `/persons` (list) redirect to `/users`; person deep-links `/persons/[slug]` and `/persons/[slug]/claim` preserved
