...
- **2026-05-22** — Completed follow-up homepage map enhancement with reusable map primitives: `components/map/BaseMap.js` now supports interactive `polygonLayers` (hover highlight, tooltip/popup helpers, click-to-fit-bounds), new `components/map/GreeceBoundaryMap.js` renders 13 Greek periphery polygons from `public/data/greece-regions.geojson`, and `components/locations/ExploreLocationsMap.js` now mounts that boundary map in the `🗺️ Εξερεύνησε Περιοχές` homepage section. Added focused coverage in `__tests__/greece-boundary-map.test.js` and documented data provenance/attribution in `doc/POLYGON_DATA.md`. Also re-synced `package-lock.json` with `package.json` to fix strict `npm ci` lockfile mismatch (`@types/react`, `csstype` entries).
