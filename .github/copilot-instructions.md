...
- **2026-05-11** — Fixed civic question comments (`entityType=civic_question`): added `civic_question` to the `Comment.entityType` ENUM in `src/models/Comment.js`; extended `getEntitySettings` in `commentController` to load `CivicQuestion` (owner=`creatorId`, enabled=`commentsEnabled`, locked=`commentsLocked`); added `civic_question` to `validEntityTypes` in both `getComments` and `createComment`; extended new-comment owner notifications to include `civic_question`; added migration `20260511000000-add-civic-question-to-comments-entity-type.js` (ALTER TYPE for Postgres, no-op for SQLite); 7 new tests in `__tests__/comments.test.js` covering GET, POST, disabled/locked enforcement, admin hide, and 404 on missing civic question
...
- **Top navigation behavior**: keep `components/layout/TopNav.js` as a plain header in normal layout flow (`relative`/non-fixed, non-sticky) with no scroll listeners/state and no scroll-direction hide/show behavior; do not add nav-height compensation to `app/layout.js` `<main>`; keep mobile menu/auth panel internal scrolling (`max-h` + `overflow-y-auto`)
