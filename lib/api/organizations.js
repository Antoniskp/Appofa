import { apiRequest } from './client.js';
import { buildQueryEndpoint } from '../utils/queryString.js';

export const organizationAPI = {
  getAll: async (params = {}) => {
    const response = await apiRequest(buildQueryEndpoint('/api/organizations', params));
    if (response?.data && !response.organizations) {
      return {
        ...response,
        organizations: response.data.organizations,
        pagination: response.data.pagination,
      };
    }
    return response;
  },

  getBySlug: async (slug) => apiRequest(`/api/organizations/${slug}`),

  create: async (data) => apiRequest('/api/organizations', {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  update: async (id, data) => apiRequest(`/api/organizations/${id}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  }),

  delete: async (id) => apiRequest(`/api/organizations/${id}`, {
    method: 'DELETE',
  }),

  getMembers: async (id) => apiRequest(`/api/organizations/${id}/members`),

  join: async (id) => apiRequest(`/api/organizations/${id}/join`, {
    method: 'POST',
  }),

  leave: async (id) => apiRequest(`/api/organizations/${id}/leave`, {
    method: 'DELETE',
  }),

  inviteMember: async (id, userId) => apiRequest(`/api/organizations/${id}/members/invite`, {
    method: 'POST',
    body: JSON.stringify({ userId }),
  }),

  approveMember: async (id, userId) => apiRequest(`/api/organizations/${id}/members/${userId}/approve`, {
    method: 'PATCH',
  }),

  removeMember: async (id, userId) => apiRequest(`/api/organizations/${id}/members/${userId}`, {
    method: 'DELETE',
  }),

  updateMemberRole: async (id, userId, role) => apiRequest(`/api/organizations/${id}/members/${userId}/role`, {
    method: 'PATCH',
    body: JSON.stringify({ role }),
  }),

  getPendingMembers: async (id) => apiRequest(`/api/organizations/${id}/members/pending`),

  getPolls: async (id, params = {}) => apiRequest(buildQueryEndpoint(`/api/organizations/${id}/polls`, params)),

  createPoll: async (id, data) => apiRequest(`/api/organizations/${id}/polls`, {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  getSuggestions: async (id, params = {}) => apiRequest(buildQueryEndpoint(`/api/organizations/${id}/suggestions`, params)),

  createSuggestion: async (id, data) => apiRequest(`/api/organizations/${id}/suggestions`, {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  getOfficialPosts: async (id, params = {}) => apiRequest(buildQueryEndpoint(`/api/organizations/${id}/official-posts`, params)),

  createOfficialPost: async (id, data) => apiRequest(`/api/organizations/${id}/official-posts`, {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  getVerificationStatus: async (id) => apiRequest(`/api/organizations/${id}/verification`),

  setVerified: async (id, isVerified) => apiRequest(`/api/organizations/${id}/verify`, {
    method: 'PATCH',
    body: JSON.stringify({ isVerified }),
  }),

  submitClaim: async (id, data) => apiRequest(`/api/organizations/${id}/claim`, {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  getPendingClaims: async (params = {}) => apiRequest(buildQueryEndpoint('/api/organizations/claims', params)),

  approveClaim: async (claimId, data = {}) => apiRequest(`/api/organizations/claims/${claimId}/approve`, {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  rejectClaim: async (claimId, data = {}) => apiRequest(`/api/organizations/claims/${claimId}/reject`, {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  getChildren: async (id) => apiRequest(`/api/organizations/${id}/children`),

  setParent: async (id, parentId) => apiRequest(`/api/organizations/${id}/parent`, {
    method: 'PATCH',
    body: JSON.stringify({ parentId }),
  }),

  getAnalytics: async (id) => apiRequest(`/api/organizations/${id}/analytics`),

  getRoles: async (id, params = {}) => apiRequest(buildQueryEndpoint(`/api/organizations/${id}/roles`, params)),

  createRole: async (id, data) => apiRequest(`/api/organizations/${id}/roles`, {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  updateRole: async (id, roleId, data) => apiRequest(`/api/organizations/${id}/roles/${roleId}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  }),

  deleteRole: async (id, roleId) => apiRequest(`/api/organizations/${id}/roles/${roleId}`, {
    method: 'DELETE',
  }),
};
