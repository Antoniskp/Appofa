...
export const ARTICLE_TYPES = ['personal', 'articles', 'news', 'video'];

/**
 * Get the display label for an article type
 * @param {string} type - Article type (personal, articles, news, video)
...
export function isCategoryRequired(type, categoriesConfig) {
  const typeConfig = categoriesConfig?.articleTypes?.[type];
  if (!typeConfig) return false;
  if ('categoryRequired' in typeConfig) return typeConfig.categoryRequired;
  return typeConfig.categories?.length > 0;
}
