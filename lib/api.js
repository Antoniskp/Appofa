...
export const authAPI = {
  ...
  searchUsers: async (params = {}) => {
    ...
    const endpoint = queryString ? `/api/auth/users/search?${queryString}` : '/api/auth/users/search';
...
export const pollAPI = {
  ...
  getAll: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiRequest(`/api/polls${queryString ? `?${queryString}` : ''}`);
  },
  
  // Get single poll with details
  getById: async (id) => apiRequest(`/api/polls/${id}`),
  
  // Get poll results
  getResults: async (id) => apiRequest(`/api/polls/${id}/results`),
  
  // Create new poll
  create: async (pollData) => apiRequest('/api/polls', {
    method: 'POST',
    body: JSON.stringify(pollData)
  }),
  
  // Update poll
  update: async (id, pollData) => apiRequest(`/api/polls/${id}`, {
    method: 'PUT',
    body: JSON.stringify(pollData)
  }),
  
  // Delete poll
  delete: async (id) => apiRequest(`/api/polls/${id}`, {
    method: 'DELETE'
  }),
  
  // Submit vote
  vote: async (pollId, voteData) => apiRequest(`/api/polls/${pollId}/vote`, {
    method: 'POST',
    body: JSON.stringify(voteData)
  }),
  
  // Add option to poll
  addOption: async (pollId, optionData) => apiRequest(`/api/polls/${pollId}/options`, {
    method: 'POST',
