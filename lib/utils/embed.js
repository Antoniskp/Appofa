...
export const EMBED_ENTITY_CONFIG = {
  polls: {
    defaultHeight: 620,
    fallbackTitle: 'Appofasi poll embed',
  },
  suggestions: {
    defaultHeight: 540,
    fallbackTitle: 'Appofasi suggestion embed',
  },
  'civic-questions': {
    defaultHeight: 620,
    fallbackTitle: 'Appofasi civic question embed',
  },
};
...
export function buildIframeEmbedCode({
  ...
  return `<iframe src="${escapeHtmlAttribute(src)}" width="${escapeHtmlAttribute(width)}" height="${escapeHtmlAttribute(height)}" style="border:0;width:100%;max-width:720px;border-radius:16px;overflow:hidden;" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" title="${escapeHtmlAttribute(title || 'Appofasi embed')}"></iframe>`;
}
