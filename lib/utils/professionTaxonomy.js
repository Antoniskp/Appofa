...
export const VALID_EMPLOYMENT_TYPES = new Set(EMPLOYMENT_TYPES.map((t) => t.id));
export const VALID_SERVICE_MODES = new Set(SERVICE_MODES.map((m) => m.id));

const EXPERTISE_TAG_LABEL_MAP = new Map(EXPERTISE_TAGS.map((t) => [t.id, t.label]));
const EMPLOYMENT_TYPE_LABEL_MAP = new Map(EMPLOYMENT_TYPES.map((t) => [t.id, t.label]));
const SERVICE_MODE_LABEL_MAP = new Map(SERVICE_MODES.map((m) => [m.id, m.label]));
...
export function getEmploymentTypeLabel(id) {
  return EMPLOYMENT_TYPE_LABEL_MAP.get(id) || id;
}

export function getServiceModeLabel(id) {
  return SERVICE_MODE_LABEL_MAP.get(id) || id;
}
