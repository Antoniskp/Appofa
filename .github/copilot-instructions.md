...
- **2026-05-14** — Added PWA install-app static page and banner: new `app/(statics)/platform/install-app/page.js` (route `/platform/install-app`) with benefit cards, per-device installation instructions (iOS Safari + Android Chrome) and FAQ; new dismissible `components/ui/InstallAppBanner.js` client component (persists dismissal via `localStorage`) exported from `components/ui/index.js`; `/platform/install-app` link added to platform page `Οδηγός Πλατφόρμας` section.
- **2026-05-14** — Homepage clarity refresh: `components/layout/HomeHero.js` now shows a fixed mission tagline above slides, guest CTAs reduced to `Εγγραφή` + `Βρες την Περιοχή σου`, logged-in CTAs simplified to location + polls (with admin/moderator-only admin link), and hero nav cards replaced by 3 value-proposition cards (`/polls`, `/suggestions`, `/locations`). `app/page.js` removed duplicate guest open-polls feed/section, merged news+articles into one `Νέα & Άρθρα` section (merged/sorted/latest 6 with per-item type pill), moved CTA banner up (after Government Snapshot + optional info), and moved Manifest Supporters to render after videos.
