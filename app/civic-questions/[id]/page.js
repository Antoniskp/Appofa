...
export default function CivicQuestionDetailPage() {
    ...
    return (
      <div className="bg-gray-50 min-h-screen py-8">
        <div className="app-container max-w-4xl">
          <EmptyState
            type="error"
            title="Civic question not found"
            description={error || 'The civic question you requested does not exist.'}
  ...
  const canVote = user && civicQuestion.status === 'open' && (!civicQuestion.deadline || new Date(civicQuestion.deadline) > new Date());
      ...
      <div className="app-container max-w-4xl space-y-6">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex flex-wrap gap-2 mb-3">
            <Badge variant={civicQuestion.status === 'open' ? 'success' : 'gray'}>{civicQuestion.status}</Badge>
            <Badge variant="primary">{civicQuestion.sourceType}</Badge>
          ...
          <div className="text-sm text-gray-600 space-y-1">
            {civicQuestion.sourceName && <p>Source: {civicQuestion.sourceName}</p>}
            ...
            {civicQuestion.location?.name && <p>Location: {civicQuestion.location.name}</p>}
            {civicQuestion.dateAsked && <p>Date asked: {new Date(civicQuestion.dateAsked).toLocaleDateString('el-GR')}</p>}
            {civicQuestion.deadline && <p>Deadline: {new Date(civicQuestion.deadline).toLocaleString('el-GR')}</p>}
          </div>
          {canEdit && (
            <button
              type="button"
        ...
        {canVote && (
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Vote</h2>
