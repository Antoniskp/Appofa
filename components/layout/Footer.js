...
export default function Footer() {
  ...
  const d = useTranslations('democracy');
  ...
  const footerColumns = [
    ...
    },
    {
      ...
      links: [
        ...
        { href: '/platform', label: d('platform'), type: 'internal' },
  ...
  return (
    <footer className="bg-charcoal text-white mt-auto" aria-label={t('menu_aria')}>
                  ...
                  <li key={link.href || `${column.title}-${index}`}>
                    {link.type === 'external' ? (
                      <a
                        ...
                        className="text-brand-border hover:text-white transition-colors text-sm"
                      >
                        {link.label}
                      </a>
                    ) : link.type === 'discord' ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center text-brand-border hover:text-indigo-400 transition-colors text-sm"
                      >
                      ...
                      <span className="text-brand-border text-sm">
                        {link.label}
                      </span>
                    ) : link.type === 'cookie' ? (
                      <button
                        onClick={() => window.dispatchEvent(new CustomEvent('open-cookie-settings'))}
                        className="text-brand-border hover:text-white transition-colors text-sm text-left"
                      >
                        {link.label}
                      </button>
                    ) : (
                      <Link href={link.href} className="text-brand-border hover:text-white transition-colors text-sm">
                        {link.label}
                      </Link>
                    )}
                  </li>
        ...
        <div className="border-t border-gray-700 mt-4 pt-4 text-center">
          <p className="text-brand-border text-sm">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-brand-border hover:text-white transition-colors"
            >
              GitHub
            </a>
            .
