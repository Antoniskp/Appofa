...
| `locations/` | 8 | CountryFundingBanner, LocationBreadcrumb, LocationCard, LocationEditForm (includes LocationModeratorManager section), LocationElectionsTab, LocationHeader (Phase 1: identity-first header, collapsible secondary metadata, sublocation preview+reveal), LocationModeratorManager (admin: add/remove moderator assignments for a location), LocationTabs (Phase 1: scanable feed cards with consistent type/status badges) |
...
- `app/locations/[slug]/page.js`, `components/locations/LocationHeader.js`, `components/locations/LocationTabs.js`: location detail Phase 1 hierarchy update — header emphasizes icon/name/type/parent and key stats, secondary metadata moved into a collapsible “Περισσότερες πληροφορίες”, sublocation chips default to first 8 with `+N ακόμα υποπεριοχές`, and tab feed cards now use stronger title/spacing hierarchy with consistent type+status badge styling for polls/news/articles/suggestions.
