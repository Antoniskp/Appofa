...
- **2026-05-01** — Phase 3 taxonomy-aware specialist discovery: added `scoreSpecialistMatch({professions, expertiseArea}, query)` to `src/utils/professionTaxonomy.js` (CJS) and `lib/utils/professionTaxonomy.js` (ESM); scoring: domain=+2, profession=+3, specialization=+4, subspecialization=+5, expertise tag=+4 per match; `userService.searchUsers` accepts optional `taxonomyQuery` (domainId, professionId, specializationId, subspecializationId, expertiseTags), applies DB-level LIKE pre-filter then in-memory scoring+ranking, returns `_relevanceScore`; `authController.searchUsers` extracts/validates taxonomy query params; `app/users/page.js` adds domain→profession→specialization cascade dropdowns; `components/UserCard.js` now renders primary profession label + up to 2 expertise tags; 22 tests in `__tests__/specialist-matching.test.js`
