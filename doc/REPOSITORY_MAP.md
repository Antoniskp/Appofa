...
- `components/layout/TopNav.js`: **stable fixed header (no scroll hide/show effects)** — nav stays `fixed top-0 left-0 right-0 z-50` and no longer toggles `translate-y`/`transition-transform` classes based on scroll direction; this keeps the top menu predictably visible on both desktop and mobile. `isScrolled` still applies `shadow-md backdrop-blur-sm` after >4px scroll (fallback `shadow-sm` at top). Mobile menu (`#mobile-menu`) keeps `max-h-[calc(100dvh-4rem)] overflow-y-auto` for independent scrolling on short viewports. Mobile auth dropdown keeps `menuClassName="w-full max-h-[55vh] overflow-y-auto"` so all menu items are reachable by scrolling within the panel. `mobileMenuItems` use `isMobileActive()` (not `isActive()`) so active items show `bg-blue-50 text-blue-600` instead of desktop `border-b-2`; mobile auth dropdown trigger has `min-w-0` + `truncate` on the text span and `flex-shrink-0` on the chevron to prevent long usernames from causing overflow; unauthenticated mobile login/register buttons are both full-width `flex w-full` blocks for consistent tap targets.
