...
- **2026-05-22** — Added reusable `Αναλυτική Χαρτογράφηση` political explorer at `/citizen-help/regions-electoral-map` using new `components/political/AnalyticalMappingExplorer.js` + existing `components/map/BaseMap.js`. Introduced externalized political datasets under `config/map-data/` (`regions.metadata.json`, `electoral-districts.metadata.json`, `regions.geojson`, `electoral-districts.geojson`) with stable `id` / `regionId` joins so metadata and polygons can evolve independently. Region/district labels now link into Appofa locations via search/type query links when metadata includes location search hints. Added focused schema/join coverage in `__tests__/political-mapping-data.test.js`.
