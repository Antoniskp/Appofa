...
- **2026-06-21** — Dependency security maintenance. Fixed all 18 vulnerabilities (17 moderate + 1 high) by adding two npm `overrides` in `package.json`: `"undici": ">=6.27.0"` (resolves 4 high-severity undici CVEs — HTTP header injection, WebSocket DoS, response queue poisoning, Set-Cookie downgrade; transitive via `sqlite3 → node-gyp`) and `"js-yaml": ">=4.2.0"` (resolves 17 moderate js-yaml quadratic-complexity DoS CVEs — GHSA-h67p-54hq-rp68; transitive via `jest → babel-plugin-istanbul → @istanbuljs/load-nyc-config`). js-yaml resolved to 5.0.0; `@istanbuljs/load-nyc-config` only calls `js-yaml.load()` which is fully compatible with 4.x/5.x. `npm audit` and `npm audit --omit=dev` both report 0 vulnerabilities. Before: 18 vulns (17 moderate, 1 high). After: 0 vulns.
