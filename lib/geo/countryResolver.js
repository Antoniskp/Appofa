...
const readCookie = (name) => {
  ...
  for (const cookie of cookies) {
    ...
    if (key === name) return decodeURIComponent(cookie.slice(eqIdx + 1).trim());
  }
  return null;
};

/**
 * Return the user's explicitly saved country code, or null if not yet chosen.
 * Reads from the `appofa_user_country` cookie.
 */
...
/** Simple IANA timezone → ISO-3166-1 alpha-2 mapping for common Greek/Cypriot zones. */
