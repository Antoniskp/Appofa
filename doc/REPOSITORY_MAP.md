...
>
> Media library update (2026-07-12): `MediaAsset` is now the unified upload model with variant metadata (`articleCover`/`thumbnail`/`avatar`), quota-aware uploads, and orphan-tracking fields. Articles now support `coverImageId` relation (while preserving `bannerImageUrl` fallback). Phase 2 adds admin media management endpoints (`GET /api/media/admin/stats`, `GET /api/media/admin/cleanup-report`) and admin page `/admin/media`, centralized reference accounting (articles + avatars + poll-option `mediaAssetId`), dry-run cleanup candidate file-status reporting, strict blocking for referenced deletions, and confirmed cleanup execution via `POST /api/media/admin/cleanup` (`confirm: "delete-orphaned-media"`). Poll options now support first-class `PollOption.mediaAssetId` (fallback `photoUrl` retained), and authenticated non-media-role users may upload only self-owned `shared`/`shared` media for their complex poll options while article-cover uploads stay role-gated. Scripts: `npm run media:cleanup` (`src/scripts/media-orphan-cleanup.js`) and `npm run media:adopt-legacy` (`src/scripts/media-adopt-legacy.js`).
...
- `components/layout/HomeHero.js`, `src/controllers/heroSettingsController.js`, `components/HomeActionLanes.js`: homepage positioning copy now leads with independent voices + civic decision tooling while remaining explicitly pluralistic (`Χώρος ανεξάρτητων φωνών · Εργαλείο αποφάσεων`). Hero fallbacks use the new framing, HeroSettings first-run seed now includes an additional organizations invitation slide (`/organizations`), and the participation grid adds a dedicated card for parties/organizations next to the independents path.
