import { apiRequest } from './client.js';
import { buildQueryEndpoint } from '../utils/queryString.js';

/**
 * Location API methods
 */
export const locationAPI = {
  getAll: async (params = {}) => {
    return apiRequest(buildQueryEndpoint('/api/locations', params));
  },
  
  getById: async (id) => {
    return apiRequest(`/api/locations/${id}`);
  },
  
  create: async (locationData) => {
    return apiRequest('/api/locations', {
      method: 'POST',
      body: JSON.stringify(locationData),
    });
  },
  
  update: async (id, locationData) => {
    return apiRequest(`/api/locations/${id}`, {
      method: 'PUT',
      body: JSON.stringify(locationData),
    });
  },
  
  delete: async (id) => {
    return apiRequest(`/api/locations/${id}`, {
      method: 'DELETE',
    });
  },
  
  link: async (entityType, entityId, locationId) => {
    return apiRequest('/api/locations/link', {
      method: 'POST',
      body: JSON.stringify({ 
        entity_type: entityType, 
        entity_id: entityId, 
        location_id: locationId 
      }),
    });
  },
  
  unlink: async (entityType, entityId, locationId) => {
    return apiRequest('/api/locations/unlink', {
      method: 'POST',
      body: JSON.stringify({ 
        entity_type: entityType, 
        entity_id: entityId, 
        location_id: locationId 
      }),
    });
  },
  
  getEntityLocations: async (entityType, entityId) => {
    return apiRequest(`/api/locations/${entityType}/${entityId}/locations`);
  },
  
  getLocationEntities: async (id) => {
    return apiRequest(`/api/locations/${id}/entities`);
  },
  uploadImage: async (id, file) => {
    const formData = new FormData();
    formData.append('image', file);
    return apiRequest(`/api/locations/${id}/image`, {
      method: 'POST',
      body: formData,
    });
  },
};

/**
 * Location Request API methods
 */
export const locationRequestAPI = {
  create: async (requestData) => {
    return apiRequest('/api/locations/requests', {
      method: 'POST',
      body: JSON.stringify(requestData),
    });
  },

  getAll: async (params = {}) => {
    return apiRequest(buildQueryEndpoint('/api/locations/requests', params));
  },

  update: async (id, data) => {
    return apiRequest(`/api/locations/requests/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  },
};

/**
 * Location Section API methods
 */
export const locationSectionAPI = {
  getAllCameras: async () => {
    return apiRequest('/api/locations/cameras');
  },

  updateCameraStatus: async (sectionId, index, isWorking) => {
    return apiRequest(`/api/locations/cameras/${sectionId}/${index}/status`, {
      method: 'PUT',
      body: JSON.stringify({ isWorking }),
    });
  },

  getSections: async (locationId) => {
    return apiRequest(`/api/locations/${locationId}/sections`);
  },

  createSection: async (locationId, sectionData) => {
    return apiRequest(`/api/locations/${locationId}/sections`, {
      method: 'POST',
      body: JSON.stringify(sectionData),
    });
  },

  updateSection: async (locationId, sectionId, sectionData) => {
    return apiRequest(`/api/locations/${locationId}/sections/${sectionId}`, {
      method: 'PUT',
      body: JSON.stringify(sectionData),
    });
  },

  deleteSection: async (locationId, sectionId) => {
    return apiRequest(`/api/locations/${locationId}/sections/${sectionId}`, {
      method: 'DELETE',
    });
  },

  reorderSections: async (locationId, order) => {
    return apiRequest(`/api/locations/${locationId}/sections/reorder`, {
      method: 'PUT',
      body: JSON.stringify({ order }),
    });
  },
};

/**
 * Location Role API methods
 */
export const locationRoleAPI = {
  getRoles: async (locationId) => {
    return apiRequest(`/api/locations/${locationId}/roles`);
  },

  upsertRoles: async (locationId, roles) => {
    return apiRequest(`/api/locations/${locationId}/roles`, {
      method: 'PUT',
      body: JSON.stringify({ roles }),
    });
  },
};

/**
 * Location Platform Role API methods (UserLocationRole join table).
 * Admin-only. Used by LocationModeratorManager on the location edit page.
 */
export const locationPlatformRoleAPI = {
  /** List all platform role assignments for a location. */
  listAssignments: async (locationId) => {
    return apiRequest(`/api/locations/${locationId}/platform-roles`);
  },

  /** Add a new platform role assignment (userId + roleKey) for a location. */
  addAssignment: async (locationId, userId, roleKey) => {
    return apiRequest(`/api/locations/${locationId}/platform-roles`, {
      method: 'POST',
      body: JSON.stringify({ userId, roleKey }),
    });
  },

  /** Remove a specific platform role assignment by its ID. */
  removeAssignment: async (locationId, assignmentId) => {
    return apiRequest(`/api/locations/${locationId}/platform-roles/${assignmentId}`, {
      method: 'DELETE',
    });
  },
};

/**
 * Electoral District Mapping API methods.
 * Manage the many-to-many relationship between locations (municipalities)
 * and electoral districts.
 */
export const electoralDistrictAPI = {
  /** Get electoral districts mapped to a location (typically a municipality). */
  getMunicipalityDistricts: async (locationId) => {
    return apiRequest(`/api/locations/${locationId}/electoral-districts`);
  },

  /** Get municipalities mapped to an electoral district. */
  getDistrictMunicipalities: async (districtId) => {
    return apiRequest(`/api/locations/${districtId}/municipalities`);
  },

  /** Add a mapping between a location and an electoral district (admin/moderator). */
  addMapping: async (locationId, electoralDistrictId) => {
    return apiRequest(`/api/locations/${locationId}/electoral-districts`, {
      method: 'POST',
      body: JSON.stringify({ electoralDistrictId }),
    });
  },

  /** Remove a specific mapping by its mapping row ID (admin/moderator). */
  removeMapping: async (locationId, mappingId) => {
    return apiRequest(`/api/locations/${locationId}/electoral-districts/${mappingId}`, {
      method: 'DELETE',
    });
  },
};

export const locationElectionAPI = {
  getElections: async (locationId) => {
    return apiRequest(`/api/locations/${locationId}/elections`);
  },

  castVote: async (locationId, roleKey, candidateUserId) => {
    return apiRequest(`/api/locations/${locationId}/elections/${encodeURIComponent(roleKey)}/vote`, {
      method: 'POST',
      body: JSON.stringify({ candidateUserId }),
    });
  },

  removeVote: async (locationId, roleKey) => {
    return apiRequest(`/api/locations/${locationId}/elections/${encodeURIComponent(roleKey)}/vote`, {
      method: 'DELETE',
    });
  },
};
