...
- **2026-04-30** — Improved rating rate-limit UX: `makeRateLimitHandler(message)` added to `rateLimiter.js` — all 429 responses now include `retryAfter` (seconds) and `resetTime` (epoch ms); `pollRoutes.js` vote limiter increased 10 → 30/hour; `lib/api/client.js` attaches `err.retryAfter`/`err.resetTime` on 429 errors; new `components/ui/RateLimitBanner.js` countdown-timer component (uses i18n `common.rate_limit_*` keys); `PollVoting.js` and `InlineSuggestionVote.js` show banner and disable vote controls while limited; 4 new tests added across api-client, polls, and inline-suggestion-vote test files
