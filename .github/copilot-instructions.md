...
- **2026-09-20** — Democracy first release: participation-first homepage, `/progress` public feed, and `Suggestion.progress` JSON timeline via `PATCH /api/suggestions/:id/progress`. Only proposal author/platform admin may publish updates; revision checks and row locking prevent lost updates. Public progress feed excludes restricted and organization-scoped proposals. Existing timeline statuses must change through the progress endpoint. Voting is explicitly advisory; hiding a name is not ballot secrecy. Administrators no longer bypass local voting eligibility (polls, civic questions, suggestions, solutions) or organization poll membership. Added EN/EL/RO `democracy` messages and migration `20260920000000-add-proposal-progress.js`. See `doc/DEMOCRACY_RELEASE.md`.

- **2026-07-12** — Media Library Phase 4 admin lifecycle hardening: added confirmed admin cleanup execution (`POST /api/media/admin/cleanup`) behind CSRF + admin role + explicit `confirm: "delete-orphaned-media"`. `/admin/media` now keeps the dry-run report as the first step, lists candidate IDs/file counts, and only then offers a destructive confirmation dialog to archive/delete eligible old orphan assets. Added EN/EL/RO cleanup action translations and regression coverage that rejected unconfirmed cleanup and verified confirmed cleanup archives old orphan assets.
- **2026-07-12** — Media Library Phase 3 follow-up: general `/api/media/upload` now allows authenticated users to upload only self-owned `shared`/`shared` media for complex poll options, while article-cover/admin-style uploads still require media roles (`admin`/`moderator`/`editor`). Upload quota responses now consistently include `remainingBytes`. Added regression coverage in `__tests__/media-library.test.js` proving regular users can upload shared poll-option media and create complex polls with their own `mediaAssetId`, but cannot upload article-cover media.
...
- **Org poll access via poll API**: enforce active `OrganizationMember` checks in `pollService` for org-scoped private poll reads/results; admins retain read access. Voting on organization polls requires active membership for everyone, including admins.
