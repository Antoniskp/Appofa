...
| `layout/` | 10 | AppShell (hides TopNav/Footer/CookieBanner on `/embed/*` while keeping normal chrome elsewhere), TopNav, Footer, HomeHero, ToastProvider, StaticPageLayout, GeoTracker, GoogleAnalytics (loads GA script via `next/script` and tracks route-change pageviews when consent is granted) |
...
| `locations/` | 13 | CountryFundingBanner (country no-content card with Geo-IP transparency panel: detected country, source, browser locale, trust level, and applied mode; plus support CTA, optional donation CTA, diaspora shortcut to Greece), ExploreLocationsMap (homepage + `/locations/greece` prefecture discovery map wrapper with pills), LocationBoundaryGeoJsonField (reusable location boundary editor with instructions, template buttons, paste/upload validation, and BaseMap polygon preview), LocationBreadcrumb, LocationCard, LocationEditForm (includes LocationModeratorManager section and map-default/boundary-color inputs), LocationElectionsTab, LocationHeader (balanced desktop two-column top box with participation-first CTA hierarchy: one primary action + compact edit icon, muted zero stat tiles), LocationMap (**updated**: renders `boundary_geojson` as interactive polygon layer with perimeter/outline when present; bare Polygon/MultiPolygon auto-wrapped; auto-fits derived polygon bounds; supports `boundary_color`; falls back to marker/default center/zoom), LocationModeratorManager (admin: add/remove moderator assignments for a location), LocationOverviewPanel (legacy summary cards component, no longer rendered in default location detail flow), LocationRelatedLocations (compact related/nearby chip layout replacing large hierarchy blocks), LocationTabs (polls/suggestions-first tab UX with compact poll-card grid and explicit `+ Ξεκίνησε ...` empty-state actions) |
| `map/` | 3 | BaseMap (shared Leaflet map with marker + overlay + polygonLayers support, plus `styleFeature` per-feature coloring hook, and compliant attribution), GreeceBoundaryMap (prefecture/region boundary polygon layer map fed by location `boundary_geojson` first with static fallback; click shows a single React info card to avoid duplicate popup boxes; supports per-prefecture `boundary_color`), LocationPickerMap (interactive click/drag coordinate picker reused by location create/edit flows) |
...
- `app/locations/[slug]/page.js`, `components/locations/LocationHeader.js`, `components/locations/LocationRelatedLocations.js`, `components/locations/LocationTabs.js`: location detail participation-first refinement — `#location-content` now appears immediately below the hero/header, `polls` + `suggestions` tabs stay visible even when empty, and `Τοπικές πληροφορίες` is omitted when no real section data exists. Header quick actions now enforce one primary participation CTA (`Ψήφισε τώρα` or `Κάνε πρόταση`) with edit demoted to a compact icon near the title, while compact related-location chips replace the large hierarchy navigation block. Poll cards inside location tabs now render in a denser responsive grid (`md:grid-cols-2`) with explicit add-action empty-state copy.
