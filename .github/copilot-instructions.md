...
- **2026-06-09** — Webcam sections now support optional exact per-camera pin coordinates via `content.webcams[].lat` / `content.webcams[].lng` in addition to optional `locationId`. `components/LocationSectionManager.js` reuses `LocationPickerMap` inside `WebcamsEditor` so moderators can drop/drag a pin per camera and fine-tune numeric lat/lng fields while keeping the linked-location selector. `src/controllers/locationSectionController.js` normalizes numeric strings, requires lat+lng together, validates ranges, and the public `GET /api/locations/cameras` feed now prefers exact camera coordinates over explicit `locationId`, then falls back to the source section location. Coverage added in `__tests__/location-sections.test.js`, `__tests__/cameras-page.test.js`, and `__tests__/location-section-manager-webcams.test.js`.
- **2026-06-09** — Added community cameras discovery. `components/layout/TopNav.js` now includes `/cameras` under the `Κοινότητα` dropdown/mobile section using `nav.cameras` i18n keys. Location `webcams` section entries now support optional `locationId` references in `content.webcams[]`; `locationSectionController` validates positive integer IDs, rejects unknown locations on create/update, and adds public `GET /api/locations/cameras` to flatten published webcam sections into camera cards with explicit `location`, fallback `sourceLocation`, and resolved `mapLocation`. New user-facing `/cameras` route renders `components/cameras/CamerasPageClient.js`: a polished map-first cameras directory using `BaseMap` markers plus cards linking to the camera stream and related location. Coverage added in `__tests__/top-nav-grouped-menu.test.js`, `__tests__/location-sections.test.js`, and `__tests__/cameras-page.test.js`.
...
- **2026-06-04** — Extended public organization identity rendering in cards. `suggestionController.getSuggestions` now includes `organization` relation data (id/name/slug/type/logo/isVerified) for public suggestion feeds, and `pollService.getAllPolls` now includes the same `organization` relation for poll listings. Added shared `OrgAvatar` (`components/organization/OrgAvatar.js`) and updated both `SuggestionCard` and `PollCard` to show organization logo/fallback icon + organization name in the footer identity row when content is `isOfficialPost` and organization metadata exists. Added focused coverage in `__tests__/suggestions.test.js`, `__tests__/polls.test.js`, `__tests__/suggestion-card-layout.test.js`, and `__tests__/poll-card-organization-identity.test.js`.
...
- **Location webcams / cameras**: the `webcams` location-section content model supports optional per-camera `locationId` plus optional exact `lat` / `lng` in `content.webcams[]`. In `locationSectionController`, normalize numeric strings, require exact pins to provide both coordinates, validate `lat` in `[-90, 90]`, validate `lng` in `[-180, 180]`, and still reject unknown `locationId` references on write. Public `GET /api/locations/cameras` is the canonical flattened feed for the `/cameras` page and should return `location` (explicit association when present), `sourceLocation` (owning section location), `exactCoordinates` (camera pin when present), and `mapLocation` resolved in this order: exact camera pin → explicit `locationId` location → source location. `components/LocationSectionManager.js` should keep using `LocationPickerMap` for moderator pin selection, while `components/cameras/CamerasPageClient.js` should keep rendering markers from `mapLocation`.
