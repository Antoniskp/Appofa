...
- **Location webcams / cameras**: the `webcams` location-section content model supports optional per-camera `locationId` in `content.webcams[]`. Validate and normalize it in `locationSectionController` (positive integer only; reject unknown locations on write). Public `GET /api/locations/cameras` is the canonical flattened feed for the `/cameras` page and should return `location` (explicit association when present), `sourceLocation` (owning section location), and `mapLocation` (explicit-or-fallback map context). On the frontend, `components/cameras/CamerasPageClient.js` is the canonical community cameras directory: map-first layout with `BaseMap` markers for cameras whose resolved map location has valid coordinates.
