...
- **Geo analytics**: use `GeoVisit` as append-only traffic telemetry (country/path/locale/sessionHash/ipAddress/userId) via non-blocking `geoTrackMiddleware` and `POST /api/admin/geo-stats/track` from `proxy.js` (forwarding cookie `token` for analytics-only decode); read `x-detected-country` fallback when `cf-ipcountry` is unavailable
