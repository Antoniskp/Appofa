...
export const imageAPI = {
  ...
  addExternal: async (imageData) => {
    return apiRequest('/api/images/external', {
      method: 'POST',
      body: JSON.stringify(imageData),
    });
  },

  getMyImages: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    const endpoint = queryString ? `/api/images/my-images?${queryString}` : '/api/images/my-images';
    return apiRequest(endpoint);
  },

  search: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    const endpoint = queryString ? `/api/images/search?${queryString}` : '/api/images/search';
    return apiRequest(endpoint);
  },
};
