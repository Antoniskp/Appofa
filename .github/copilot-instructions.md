...
- **Location boundary GeoJSON workflow**: store optional per-location polygons in `Location.boundary_geojson` (JSON). Accept GeoJSON roots `FeatureCollection`, `Feature`, or direct geometry objects, but enforce geometry type support to `Polygon`/`MultiPolygon` only. Location create/edit UIs provide instructions, starter templates, paste/upload (`.geojson`/`.json`), validation, and preview. `LocationMap` renders `boundary_geojson` as an interactive polygon layer with perimeter/outline (via `BaseMap.polygonLayers`) when present; bare Polygon/MultiPolygon geometries are auto-wrapped in a Feature before being passed to L.geoJSON. The location detail page shows the map section when only `boundary_geojson` is present (no lat/lng required). The homepage `GreeceBoundaryMap` renders the 13 Greek prefecture/region polygons from `/public/data/greece-regions.geojson`.
