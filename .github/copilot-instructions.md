...
- **2026-05-23** — Homepage map now uses per-location `boundary_geojson` from prefecture Location records instead of the static `public/data/greece-regions.geojson` file. `GreeceBoundaryMap` accepts a `prefectures` prop, builds a FeatureCollection from location boundaries (normalizing FeatureCollection/Feature/bare Polygon/MultiPolygon), and falls back to the static file only when no location has a boundary. `ExploreLocationsMap` renders a row of clickable prefecture pills below the map. Homepage no longer shows the 3 top location cards; location discovery state replaced with `prefectures` state (`type=prefecture&limit=50`). `HomepageSection` now suppresses EmptyState when `emptyTitle` is falsy. Tests updated in `__tests__/greece-boundary-map.test.js`.
