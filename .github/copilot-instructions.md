...
- **2026-05-01** — Phase 3 profession taxonomy: specialist discovery/filtering/matching; `scoreSpecialistMatch` added to `src/utils/professionTaxonomy.js` (CJS) and `lib/utils/professionTaxonomy.js` (ESM) with additive scoring (domain=2, profession=3, specialization=4, subspecialization=5, expertise tag=4); `searchUsers` in `userService` now accepts `taxonomyQuery` object (`domainId`, `professionId`, `specializationId`, `subspecializationId`), applies JSON LIKE filters, scores/sorts results by relevance when any taxonomy filter is active, includes `professions` in search attributes, returns `_relevanceScore` on results; `authController.searchUsers` forwards new query params; `/app/users/page.js` adds domain → profession cascade dropdowns (resets professionId when domainId changes); `UserCard` shows primary profession label + up to 2 expertise tags; 26 new tests in `__tests__/specialist-matching.test.js`
...
- **Profession taxonomy**: profession/expertise data uses canonical v2 format: `{domainId, professionId, specializationId?, subspecializationId?}` objects in `User.professions` (JSON array); `User.expertiseArea` stores kebab-case tag IDs from `src/data/expertiseTags.json`; `src/utils/professionTaxonomy.js` (CJS) and `lib/utils/professionTaxonomy.js` (ESM) are the single normalization boundary — always use `normalizeLegacyProfession`/`normalizeExpertiseTags` for legacy v1 data (`{categoryId,...}` / string labels) before `validateProfessionalIdentity`/`validateExpertiseTagIds`; `User` model getters apply normalization automatically on read; do NOT add per-file legacy compat logic
- **Specialist matching / scoring**: use `scoreSpecialistMatch({ professions, expertiseTags }, query)` from `src/utils/professionTaxonomy.js` (CJS) or `lib/utils/professionTaxonomy.js` (ESM) for relevance scoring; weights: domain=2, +profession=3, +specialization=4, +subspecialization=5, expertise tag=4 per tag; `userService.searchUsers` accepts a `taxonomyQuery` object (`{domainId, professionId, specializationId, subspecializationId}`) and applies JSON LIKE filters + in-memory scoring/sort when any field is set; `GET /api/auth/users/search` forwards these params from query string; `/app/users/page.js` provides domain → profession cascade dropdowns; `UserCard` renders primary profession label and up to 2 expertise tags
