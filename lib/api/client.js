...
const CSRF_REFRESH_SUCCESS = 'success';
const CSRF_REFRESH_FAILED = 'failed';
const CSRF_REFRESH_RATE_LIMITED = 'rate_limited';
 ...
 * - 'success' when refresh succeeds
 * - 'rate_limited' when the endpoint returns 429
 * - 'failed' on auth/network failure
 * Also serves as a session liveness check — if the JWT is expired the
 * server will return 401 and this will return 'failed'.
 */
async function refreshCsrfToken() {
  try {
    ...
    if (response.status === 429) {
      return CSRF_REFRESH_RATE_LIMITED;
    }
    return response.ok ? CSRF_REFRESH_SUCCESS : CSRF_REFRESH_FAILED;
  } catch {
    return CSRF_REFRESH_FAILED;
  }
}
  ...
  if (!response.ok) {
    ...
    if (!_isRetry && (isCsrfFailure || isTokenMissing)) {
      ...
      if (!isAuthPage) {
        ...
        if (refreshed === CSRF_REFRESH_SUCCESS) {
        ...
        if (refreshed !== CSRF_REFRESH_RATE_LIMITED && typeof window !== 'undefined') {
    ...
    if (response.status === 429 && typeof data === 'object' && data !== null) {
      if (data.retryAfter != null) err.retryAfter = data.retryAfter;
      if (data.resetTime != null) err.resetTime = data.resetTime;
    }
    throw err;
  }
