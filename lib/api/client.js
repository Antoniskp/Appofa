 ...
 * Returns true on success, false on auth/network failure, and 'rate_limited'
 * when the CSRF refresh endpoint is rate-limited.
  ...
  if (!response.ok) {
    ...
    if (!_isRetry && (isCsrfFailure || isTokenMissing)) {
      ...
      if (!isAuthPage) {
        ...
        if (refreshed !== 'rate_limited' && typeof window !== 'undefined') {
