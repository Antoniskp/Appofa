...
- **Top navigation behavior**: keep `components/layout/TopNav.js` fixed and always visible (`fixed top-0 ...`) with no scroll-direction hide/show (`translate-y`/`transition-transform`); `isScrolled` may still add subtle shadow/backdrop styling; keep `app/layout.js` `<main className="... pt-16">` spacing and mobile menu/auth panel internal scrolling (`max-h` + `overflow-y-auto`)
